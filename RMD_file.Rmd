...
colnames(df_top10) <- c("gene", "w", "pos_sites", "total_sites", "site_density")
df_top10_f <-df_top10[1:10,]
df_top10_f <- select(df_top10_f, c(gene, w, site_density))
df_top10_f$site_density <- round(df_top10_f$site_density, round(2)) # Round to 2 decimals

#Order by w value
df_s <- df_top10_f[order(df_top10_f$w, decreasing = T), ]
gene_order = c(df_s$gene)
df_top10_f$gene <- factor(df_top10_f$gene, levels = rev(gene_order))

#Plot
dNdS_top10 <- ggplot(df_top10_f, aes(x = gene, y = w)) +
