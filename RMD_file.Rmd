...
5. Inferential Statistics (part I)
    + Correlation between nucleotide diversity (π) and dNdS ratio (ω)  
&nbsp;
...
We are asking if the nucleotide diversity (index that indicates the level of variability of a nucleotide sequences across a set of species) is correlated with the index of positive selection (ω). In the case of the Oxytocin Signalling Pathway, this is how the data looks.
```{r, message=FALSE, warning=FALSE}
library("ggplot2", verbose = F)
library("dplyr", verbose = F)
# Load data
...
By using the flowers dataset ([Intro2R book](intro2r.com)) we take the variable **weight** as response to the predictors **height**, **leafarea**, **shootarea**, **flowers**
```{r}
...
PC1 and PC2 explain almost 80% of the total variance, whereas PC4 only adds about 5% of it.
